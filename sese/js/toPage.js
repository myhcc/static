function page_ctrl(data_obj,type) {
  ...
  function page_even() {
    ...
    var append_html='<a class="prev_page">上一页</a>';
    for(var i=0;i<total_page-1;i++){
      if(total_page>8&&current_page>6&&i<current_page-3){
        if(i<2){
          append_html+='<a href="/type/'+type+'/'+(i+1)+'.html" class="page_num">'+(i+1)+'</a>';
        }
        else if(i==2){
        ...
        if(i==current_page-1){
          append_html+='<a href="/type/'+type+'/'+(i+1)+'.html" class="page_num current_page">'+(i+1)+'</a>';
        }
        else{
          append_html+='<a href="/type/'+type+'/'+(i+1)+'.html" class="page_num">'+(i+1)+'</a>';
        }
    ...
    append_html+='<a class="next_page">下一页</a><span class="page_total">共 '+total_page+' 页, 到第</span><input class="input_page_num" type="text" value="'+inp_val+'"><span class="page_text">页</span><button class="to_page_num">确定</button>';
